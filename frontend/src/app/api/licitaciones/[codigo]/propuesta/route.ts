...
export async function GET(
  ...
  try {
    backendResponse = await fetch(
      `${INTERNAL_API}/api/v1/licitaciones/${encodeURIComponent(codigo)}/propuesta`,
      {
        headers: {
  ...
  if (!backendResponse.ok) {
    const errorBody = (await backendResponse.json().catch(() => ({
      detail: "Error al obtener el borrador",
    }))) as { detail?: string }
    return NextResponse.json(
      { detail: errorBody.detail ?? "Error al obtener el borrador" },
